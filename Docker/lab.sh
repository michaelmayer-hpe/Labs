...
for i in 0; do
    ...
    #for j in 1; do
        ...
        az network nsg rule create -g LabDck -n ALLOW-HTTP --nsg-name gr\$i-vm\${j}NSG --access allow --direction inbound --protocol tcp --destination-address-prefix '*' --destination-port-range 80 --source-address-prefix '*' --source-port-range '*' --description "Allow inbound HTTP traffic" --priority 1010
    done
done
sleep 100000
