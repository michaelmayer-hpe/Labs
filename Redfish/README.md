...
This lab purpose is to install and use Redfish to become familiar with Linux based containers and handle some of the common use cases around it.
...
## Reference documents
When dealing with the installation and configuration of Redfish, the first approach is to look at the reference Web site http://docker.io/: 

Estimated time for the lab is placed in front of each part.

# Environment setup
Estimated time: 15 minutes
## Redfish installation
Redfish is available externaly from http://docs.docker.com/linux/step_one/ or using your distribution packages, or from github at https://github.com/docker/docker
Version 1.7 is  the current stable release.
As we'll work on an Ubuntu environment for the Lab, you may want to use apt to do the installation of Redfish with all its dependencies. As Ubuntu provides an old version of docker, we will use a ppa providing a more up to date version:
...
`#` **`docker --version`**
```
Redfish version 1.7.0, build 0baf609
```
`#` **`docker info`**
```
Containers: 0
...
Redfish and the iLO RESTful API on ProLiant Gen9 severs
HPE Server division—V gre-1.41
March 16
Hands on Lab
...
Introduction
This lab explores the Redfish 1.0 standard and the iLO RESTful API available in HPE ProLiant Gen9 servers via multiple means.
...
Having some knowledge in the technologies below might also help:
UEFI in HP ProLiant Gen9 servers
Linux shells and scripting
Python scripting
PowerShell scripting

However, if you do not have all these optional skills, don’t worry; the lab is very step-by-step driven. You will be able to complete it anyway.
Content and exercises
This lab intends to be a complement (not substitute) to the following public documents:
Managing HP Servers Using the HP RESTful API
HP RESTful API Data model Reference for iLO4
It presents five means for retrieving and setting parameters from and in HP ProLiant Gen9 servers via the HP RESTful API for iLO:
1. A web browser with associated extension
2. The public and free hprest tool
3. wget / curl tools
4. PowerShell commands
5. A python script
The following exercises are proposed for each mean:
1. Get Properties
6. Set a property and/or perform an action
Required software
Before starting the lab exercises, your lab access station must meet the following 2 requirements:
The Firefox browser with the HttpRequester extension (See below for installation tips). 
NOTE: Google Chrome with the POSTMAN extension can be used as well. However, screen snapshots in this document have been performed with HttpRequester.
An SSH client (PuTTY on Windows clients is OK)
References:
...
In this lab, you will have to access a ProLiant Gen9 server with RHEL 7 installed on an internal disk. You will have privileged access to its iLO as well as to the operating system.
We apology for not providing a pre-installed Windows OS due to a very limited time for the setup and the reset of this infrastructure. 
Your environment
Ask the instructor for your name/number. Team names/numbers go from lab1 to lab10.
iLO Credentials: demopaq / password 
Linux credentials: root / password 
If X is your lab number, then, your corresponding iLO address is: 10.3.222.10X 
The Linux OS address will be: 10.3.222.8X
Test and set your iLO access:
From your station, in order to populate your ssh/known_host, Open a PuTTY session toward your iLO at 10.3.222.10X where X is your lab number.



Answer Yes to the Security Alert (if needed) and login as demopaq / password:
If the login is successful, exit from the iLO:
</>hpilo-> exit
Test and set your browser:
Both POSTMAN for Chrome and HttpRequester for Firefox extensions can be used for this lab. We choose to use the HttpRequester extension because of its installation flexibility in non-connected environments.
Open the Firefox browser (if not already done) and set it as your default browser, if asked. If you don’t see the double arrow icon in the top right corner, notify the instructor:

From Firefox, open a session toward our iLO at http://10.3.222.10X (where X is your lab number). 
If needed, click on Advanced and then on Add Exception… then Confirm Security Exception:

Log into the iLO using demopaq / password as credentials. Your server should be in the OFF state. If not, tell the instructor:
...
In these exercises, we use only the Basic Authentication mechanism primarily designed for single requests. Full session states are also supported by the iLO REST Interface, and explained in the Managing HP Servers Using the RESTful API for iLO document present on your Desktop. 
...
Note: In addition to the Systems link, the data model proposes others like Chassis and Managers. The description of their content is explained in the Managing HP Servers Using the iLO RESTful API document.
...
HP provides a suite of PowerShell Cmdlets in the PowerShell Gallery. The y can be used with PS Version 5.0 coming with the Microsoft Windows Management Framework 5.0 (WMF 5) or natively with Windows10 or Windows server 2016. 
...
Feel free to test other examples or go to the next exercise.
Python SDK
Like PowerShell, the python language and its multiple web and security related modules provides a perfect eco-system for creating RESTful for iLO programs. The following script is for didactic purposes only, and HPE does not supports it. Use it with care.
...
Conclusion
The iLO RESTful API provides a rich set of means to display and modify HP ProLiant Gen9 servers. Just choose the one suiting best your needs: browser extension, hprest, wget/curl or python. For Windows users, PowerShell is an alternative as well.
